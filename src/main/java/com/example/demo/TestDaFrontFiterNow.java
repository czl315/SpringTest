 ...
 *
 */
public class TestDaFrontFiterNow {
	/**
	 * @param args
	 */
	public static void main(String[] args) {
		int back1limit = 12;// 前区第1个1-12
		int back2limit = 12;// 前区第2个2-12,第二个不能小于第一个
		/**
		 * 前区所有可能组合
		 */
		List<BackBall> backBalls = backAllComb(back1limit, back2limit);
		System.out.println("前区所有可能组合size：" + backBalls.size());
		/**
		 * 获取n个前区结果
		 */
		int limitCount = 1000;
		...
		System.out.println("获取2018年至今" + limitCount + "个前区结果size：" + lastBackBalls.size());

		for (Iterator<BackBall> iterator = backBalls.iterator(); iterator.hasNext();) {
			BackBall backBallAll = (BackBall) iterator.next();
			String showStr = "前区：" + backBallAll.getBackBall1() + "，" + backBallAll.getBackBall2();
